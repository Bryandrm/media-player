...
- **Playlists**: CRUD + add/remove tracks + sidebar UI + rename inline (doble-click). `getQueue()` lee de la playlist seleccionada → NEXT/PREV/shuffle navegan dentro de ella. La vista de playlist se mantiene en sync con la library (identify/clean/scan/letras refrescan ambas). Reorder + smart playlists quedan para polish.
