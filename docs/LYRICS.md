...
4. ✅ **Alignment score + mismatch detection (2.c.4b)** — **resuelto** (2026-06-19). **Nivel 1:** `align_track` calcula promedio de `word.score` (0..1) de los word timings y lo persiste en `lyrics.alignment_score` (migración `20260619000001`). Score bajo (< 0.5) indica mismatch LRC↔audio. **Nivel 2:** script `mismatch_detect.py` transcribe el audio con WhisperX, convierte LRC y transcripción a fonemas IPA vía `phonemizer` (espeak-ng), y compara con Levenshtein normalizado por línea. Botón **CHECK QUALITY** en el panel de lyrics muestra score overall + líneas con mismatch (<50%). Deps: `phonemizer` (`pipx inject whisperx phonemizer`) + `espeak-ng`. Fallback a texto raw si phonemizer no está.
...
**Conclusión:** el modal de 2.c.1 es escalera de emergencia, no la solución de UX. Con smart cascade (2.c.4a) + mismatch detection (2.c.4b niveles 1+2) shipped, el flujo es: el cascade elige la mejor fuente por confidence → el usuario puede verificar calidad con CHECK QUALITY → líneas malas se identifican automáticamente por fonética.

**Hacia el módulo de edición (T1 en [BACKLOG.md](./BACKLOG.md)):** la bandera evolucionó a una tarea formal — un módulo dedicado de edición de lyrics. **inc.1 ✓ (2026-06-23): flag visual inline** — las líneas malas de CHECK QUALITY se marcan **dentro** del panel synced (borde accent + `⚠NN%` + lo que transcribió el audio), con el score per-línea **persistido** (`lyrics.mismatch_lines`, migración `20260623000001`) y refrescado por track. **Pendiente:** inc.2 edición inline por línea, inc.3 auto-refetch de otro provider ante score bajo (cerrar el "actuar automáticamente"), e integración con "Contribute to LRCLIB" (punto 5).
