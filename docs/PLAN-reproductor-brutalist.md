...
- [ ] **Tracking de `play_count` + `last_played_at`** — hoy las columnas existen pero **nunca se incrementan** al reproducir (gap detectado el 2026-06-18 al probar smart playlists). Falta un comando Rust (`track_mark_played` o similar) disparado desde el player cuando un track se considera "reproducido" (ej: ≥50% o ≥30s). Desbloquea las reglas smart `play_count` / `played_within_days` (hoy muertas) + habilita un "Most Played" real. Ver [ADR-034 "Caveat de datos"](DECISIONS.md#adr-034--smart-playlists-motor-multi-regla-con-query-builder-dinámico).
